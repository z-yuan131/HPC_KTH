...
//double fx(double *Q, double **fq, int m, int n, int j) {
...
*/
void laxf_scheme_2d(double *Q, double **fxx, double **fyy, double **nFxx, double
**nFyy,
  ...
  for (i = 1; i < n; i++) {
    ...
    for (j = 1; j < m; j++)
      for (k = 0; k < cell_size;  k++){
        nFxx[k][j] = 0.5 * ((fxx[k][j-1] + fxx[k][j]) - dx/dt * (Q(k, j, i) - Q(k, j-1, i)));
      }

    for (j = 1; j < m-1; j++)
      for (k = 0; k < cell_size;  k++)
        Q(k, j, i) = Q(k, j, i)  - dt/dx * ((nFxx[k][j+1] - nFxx[k][j]));
  }
  ...
  for (i = 1; i < m; i++) {
    ...
    for (j = 1; j < n; j++)
      for (k = 0; k < cell_size; k++)
        nFyy[k][j] = 0.5 * ((fyy[k][j-1] + fyy[k][j]) - dy/dt * (Q(k, i, j) - Q(k, i, j -1)));

    for (j = 1; j <  n-1; j++)
      for (k = 0; k < cell_size; k++)
        Q(k,i,j) = Q(k,i,j) -  dt/dy * ((nFyy[k][j+1]  -  nFyy[k][j]));
  }
...
void solver(double *Q,
  ...
  #pragma omp parallel
  {
    /* dynamic memory allocation, which is very important private flux varibles*/
    double **fxx = (double **) malloc(cell_size * sizeof(double *));
    double **fyy = (double **) malloc(cell_size * sizeof(double *));
    double **nFxx = (double **) malloc(cell_size * sizeof(double *));
    double **nFyy = (double **) malloc(cell_size * sizeof(double *));

    fxx[0] = (double *) malloc(cell_size * m * sizeof(double));
    fyy[0] = (double *) malloc(cell_size * n * sizeof(double));
    nFxx[0] = (double *) malloc(cell_size * m * sizeof(double));
    nFyy[0] = (double *) malloc(cell_size * n * sizeof(double));
  ...
  for (int i = 0; i < steps; i++) {
    ...
    #pragma omp for lastprivate(j,k)
    for (j = 1; j < n - 1 ; j++) {
      for (k = 0; k < cell_size; k++) {
    ...
    laxf_scheme_2d(Q, fxx, fyy, nFxx, nFyy, m, n, dx, dy, dt);
